...
impl<'a> Text<'a> {
    ...
    pub fn with_offset(self, offset: bool) -> Self {
        Self { offset, ..self }
    }
...
impl<'a> Shape for Text<'a> {
    fn bounds(&self, _cache: &DrawingCache) -> Rect {
        ...
        let ofs = if self.offset {
            Offset::y(self.font.text_height() - self.font.text_baseline())
        } else {
            Offset::zero()
        };
        let size = Offset::new(w, h);
        Rect::from_top_left_and_size(size.snap(self.pos, self.align) - ofs, size)
    }
